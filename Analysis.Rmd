...
Test ne može potvrditi vezu medija i okvira.
...
ggplot(df_monthly, aes(x = okvir_nazivi, y = n, fill = MONTH)) +
  ...
  scale_fill_grey(
    ...
    name   = "",                         # legend title
    labels = c("1.mjesec 2023", "12.mjesec 2022")           # legend item labels
  ) +
  labs(
    x     = "Medijski okvir",
    y     = "Broj članaka",
...
```{r okvir-vs-time, echo=F, eval=T}
okvirVStime <- tribble(
  ~okvir_nazivi,                      ~MONTH,     ~n,
  "Ekonomski okvir",                  "January",  11,
  "Ekonomski okvir",                  "December", 10,
  "Politički okvir",                  "January",  10,
  "Politički okvir",                  "December",  2,
  "Socijalni okvir",                  "January",  33,
  "Socijalni okvir",                  "December",  5,
  "Nacionalni/suverenitetni okvir",   "January",   2,
  "Moralno/etički okvir",             "December",  1,
  "Tehnološko-administrativni okvir", "January",  78,
  "Tehnološko-administrativni okvir", "December", 75,
  "Ostalo", "January",  8,
  "Ostalo", "December", 1
  # The 'Ostalo' category from your previous example is missing here.
  # The code will handle missing combinations correctly.
)

# 3. Transform the data from long to wide format (create a contingency table)
# The test functions need a matrix-like structure.
contingency_table <- okvirVStime %>%
  pivot_wider(names_from = MONTH, values_from = n, values_fill = 0) %>%
  # pivot_wider will create 'NA' for missing combinations. 
  # We replace them with 0 as 'NA' means a count of zero.
  # The `values_fill = 0` argument does this for us automatically.
  column_to_rownames("okvir_nazivi") # Set frame names as row names for the test
...
# 5. Perform Fisher's Exact Test (recommended due to low counts)
# This test is more accurate for tables with cells containing low values.
...
Postoji veza između medijskih okvira i vremena objave članaka.
...
```
Test je pokazao da postoji statistički značajna razlika u broju izvora između različitih medijskih okvira.
...
df %>%
  ...
  ggplot(aes(
    ...
    scale_fill_grey(
      ...
      name   = "Sentiment",
      breaks = c("1","2","3"),
      labels = c("Pozitivan","Negativan","Neutralan")
    ) +
    labs(
      x     = "Medijski okvir",
      y     = "Broj članaka",
...
```{r}
#Vizualni prikaz subjektivnog sentimenta u cijelom korpusu
...
# 1. Create the data as a matrix.
# The rows represent OKVIR and the columns represent sentiment (1, 2, 3).
contingency_data <- matrix(
  c(14, 1, 6,    # OKVIR 1
    ...
    2, 25, 126,  # OKVIR 6
    3,  0, 6),   # OKVIR 7
  nrow = 7,         # There are 7 rows (for 7 OKVIR levels)
  byrow = TRUE      # Fill the matrix row-by-row
)

# It's good practice to name the dimensions for clarity
rownames(contingency_data) <- paste("OKVIR", 1:7)
colnames(contingency_data) <- paste("Sentiment", 1:3)

# View the final contingency table
print("Contingency Table:")
print(contingency_data)

# 2. Perform the Chi-Square Test of Independence
chi_square_result <- chisq.test(contingency_data)

print("Chi-Square Test Result:")
print(chi_square_result)


# 3. Check for low expected counts and consider Fisher's Exact Test
# The Chi-Square test will likely produce a warning due to the zeros.
# Fisher's test is more accurate in these cases.
# Note: Fisher's test can be computationally intensive on larger tables, but is fine here.
fisher_result <- fisher.test(contingency_data, 
                             simulate.p.value = TRUE, 
                             B = 10000 ) # Number of simulations for p-value approximation)
...
```
Test je pokazao da postoji statistički značajna razlika u sentimentu između različitih medijskih okvira.
...
```
Test je pokazao da ne ostoji statistički značajna razlika u dosegu između različitih medijskih okvira.
...
```{r}
df %>% 
  # sum comments by date and okvir
  group_by(DATE, okvir_nazivi) %>% 
  summarise(total_comments = sum(KOMENTARI, na.rm = TRUE), .groups = "drop") %>% 
  # plot a line per okvir
  ggplot(aes(x = DATE, y = total_comments, color = okvir_nazivi)) +
    geom_line(size = 1) +
    labs(
      title = "Time Series of Comments per Okvir",
      x     = "Date",
...
```{r}
df %>%
  # 1) Sum comments per day and okvir
  group_by(DATE, okvir_nazivi) %>%
  summarise(total_comments = sum(KOMENTARI, na.rm = TRUE), .groups = "drop") %>%
  # 2) Plot with one facet per okvir
  ggplot(aes(x = DATE, y = total_comments)) +
    geom_line(color = "black", size = 0.8) +
    facet_wrap(~ okvir_nazivi, scales = "free_y", ncol = 2) +
    labs(
      title = "Komentari kroz vrijeme po okviru",
      x     = "Datum",
      y     = "Broj komentara"
    ) +
    theme_bw() +
...
df %>%
  ...
  ggplot(aes(x = DATE, y = total_comments)) +
    geom_line(color = "black", size = 0.8) +
    facet_wrap(~ okvir_nazivi, scales = "free_y", ncol = 2) +
    # 3) Numeric date labels instead of month names
    scale_x_date(
      date_breaks = "1 month",
      date_labels = "%Y-%m-%d"
    ) +
...
Test je pokazao da postoji statistički značajna razlika u broju komentara između različitih medijskih okvira.
...
df %>%
  count(okvir_nazivi, `TIP NASLOVA`) %>%
  ggplot(aes(
    x    = okvir_nazivi,
    y    = n,
    fill = factor(`TIP NASLOVA`)
    ...
    labs(
      x     = "",
      y     = "Broj članaka",
      title = "Članci po okviru i tipu naslova"
    ) +
    theme_minimal() +
    theme(
      axis.text.x        = element_text(angle = 45, hjust = 1),
...
contingency_table2 <- df %>% 
  count(OKVIR, `TIP NASLOVA`) %>%
  pivot_wider(
    names_from = `TIP NASLOVA`,
    values_from = n,
    values_fill = 0
  ) 


counts_mat <- as.matrix(contingency_table2[ , -1])
storage.mode(counts_mat) <- "numeric"
rownames(counts_mat) <- contingency_table2[[1]]   # first column = OKVIR

# sanity check
stopifnot(all(counts_mat >= 0), all(is.finite(counts_mat)))

# χ²:
chisq.test(counts_mat)
```

Postoji statistički značajna razlika u broju naslova između različitih medijskih okvira.
...
n_tokenTidy %>%
  ...
  #remove rows where stemmed_word contains „poslovn“ i „dnevnik“
  filter(!grepl("poslovn|dnevnik", word, ignore.case = TRUE))-> n_tokenTidy
...
top_words_per_okvir <- tidy_text_ %>%
  ...
  #remove rows where stemmed_word contains „poslovn“ i „dnevnik“
  filter(!grepl("poslovn|dnevnik", stemmed_word, ignore.case = TRUE))
