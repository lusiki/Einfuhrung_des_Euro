...
```{r echo=F, eval=F, message=F , warning= FALSE, fig.width=16, fig.height=25}
...
```{r echo=F, eval=F, message=F , warning= FALSE, dpi = 900, fig.height=10, fig.width=10}
...
```{r echo=F, eval=T, message=F , warning= FALSE, dpi = 900, fig.height=10, fig.width=10}

# 1) define your 7 OKVIR names
okviri_all <- c(
  "Ekonomski okvir",
  "Politički okvir",
  "Socijalni okvir",
...
df <- df %>%
  # assume OKVIR is an integer 1–7; if it’s character, wrap as.integer()
  mutate(
    okvir_nazivi = factor(
      as.integer(OKVIR),
...
df %>%
  count(MEDIJ, okvir_nazivi) %>%
  group_by(MEDIJ) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
  ggplot(aes(x = fct_reorder(MEDIJ, -prop), y = prop, fill = okvir_nazivi)) +
  geom_col(position = "stack") +
  scale_fill_grey(start = 0.2, end = 0.8) +
  coord_flip() +
  labs(
    x = "Medij",
    y = "Udio medijskog okvira ",
    fill = "Medijski okvir",
    title = ""
  ) +
  theme_minimal()
...
```{r okvir-vs-hour-plot, echo=F, eval=T, message=F , warning= FALSE, fig.width=7, fig.height=4}
...
```{r  echo=F, eval=T, message=F , warning= FALSE, dpi = 900, fig.height=10, fig.width=10}
df %>%
  # add the new factor variable with labels
  mutate(
    okvir_nazivi = factor(
      as.integer(OKVIR),
      levels = seq_along(okviri_all),
  ...
  # compute proportions of SENTIMENT within each okvir_nazivi
  count(okvir_nazivi, SENTIMENT) %>%
  group_by(okvir_nazivi) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
  # plot in black & white with custom SENTIMENT labels
  ggplot(aes(x = okvir_nazivi, y = prop, fill = factor(SENTIMENT))) +
    geom_col(position = "stack") +
    scale_fill_grey(
      start  = 0.2,
      end    = 0.8,
    ...
    scale_x_discrete(limits = okviri_all) +
    labs(
      x     = "",
      y     = "Udio sentimenta",
      title = ""
...
ggplot(df, aes(x = OKVIR, y = COMMENT_COUNT)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6) +
...
```{r  dpi = 900, fig.height=10, fig.width=10}
df <- df %>%
  mutate(
    DATE_BEFORE_AFTER = if_else(
...
df %>%
  count(okvir_nazivi, DATE_BEFORE_AFTER) %>%
  group_by(okvir_nazivi) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup() %>%
  ggplot(aes(x = okvir_nazivi, y = prop, fill = DATE_BEFORE_AFTER)) +
    ...
    labs(
      ...
      title = "Articles Before vs. After Jan 1, 2023 by Frame"
    ) +
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1)
...
```{r}
tab <- table(df$okvir_nazivi, df$DATE_BEFORE_AFTER)
chisq.test(tab)
```
